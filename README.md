...
## Seal and sign off an external final GDS

To add only the GF180 seal ring to an already-complete external GDS (without
using the padring generated by this repository), run:
...
and the output is `final/chip_top.gds.gz`. The Wafer.Space PDK and precheck are
cloned at pinned commits under the ignored `.signoff/` directory. The import
step is fail-closed: it requires one top cell, 0.001 µm DBU, geometry within the
selected slot, an exact post-seal slot boundary, a `GUARD_RING_MK` seal marker,
and proof that no incoming geometry was dropped.

To use another completed layout:

```sh
make signoff-final FINAL_GDS_SOURCE=/absolute/path/design.gds \
  FINAL_GDS_TOP=chip_top FINAL_GDS_SLOT=1x0p5
```
